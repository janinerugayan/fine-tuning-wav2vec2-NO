...
def compute_masd_loss_ver2(nbest_log_distribution, asd_scores, normalized_score=0):
    # Computes log distribution
    # (n, b) -> (b,): log( p1+p2+...+pn ) = log( exp(log_p1)+exp(log_p2)+...+exp(log_pn) )
    sum_nbest_log_distribution = torch.logsumexp(nbest_log_distribution, 0)
...
class Seq2seqMASDLoss(torch.nn.Module):
    ...
    def forward(
        ...
        masd_loss = compute_masd_loss_ver2(nbest_log_distribution, asd_scores_tensor, normalized_score=self.normalized_score)  # True for MWER trial
